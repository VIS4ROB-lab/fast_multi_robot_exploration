...
class SwarmTfSim {
  ...
  SwarmTfSim(ros::NodeHandle& nh) {
    ...
    init_sub_ = nh.subscribe("/pcl_render_node/init", 10,
                             &SwarmTfSim::initPosCallback, this);
    basecoor_timer_ =
        nh.createTimer(ros::Duration(1.0), &SwarmTfSim::swarmTfCallback, this);
    ...
    init_pts_[1] = Eigen::Vector3d(
        0, 0, 0);  // Drone 1's frame is aligned with world frame
  }
