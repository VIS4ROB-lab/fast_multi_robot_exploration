...
void odom_callback(const nav_msgs::Odometry::ConstPtr& msg) {
  ...
  if (config.enable_drift_odom) {
    ...
    drift_vel = ypr_to_R(drift_pose.rows(3, 5)) *
                trans(ypr_to_R(pose.rows(3, 5))) * vel;
  ...
  if (config.enable_noisy_odom) {
    ...
    noisy_odom.pose.covariance[(0 + 3) + (0 + 3) * 6] =
        config.stdyaw * config.stdyaw;
    noisy_odom.pose.covariance[(1 + 3) + (1 + 3) * 6] =
        config.stdrp * config.stdrp;
    noisy_odom.pose.covariance[(2 + 3) + (2 + 3) * 6] =
        config.stdrp * config.stdrp;
...
void config_callback(so3_disturbance_generator::DisturbanceUIConfig& _config,
                     uint32_t level) {
  config = _config;
}
