...
void map2d_callback(const nav_msgs::OccupancyGrid::ConstPtr &msg) {
  ...
  if (grids2d.maps.size() == 0) {
    ...
    maps2dCnt++;
  } else if (grids2d.maps.back().info.map_load_time !=
...
void scan_callback(const sensor_msgs::LaserScan::ConstPtr &msg) {
  ...
  scanMat = ypr_to_R(pose.rows(3, 5)) * scanMat +
            pose.rows(0, 2) * ones<rowvec>(scanMat.n_cols);
  // Update map
  for (int k = 0; k < scanMat.n_cols; k++)
    maps3d[maps3dCnt - 1].SetOccupancyFromWorldFrame(
        scanMat(0, k), scanMat(1, k), scanMat(2, k), PROB_LASER_OCCUPIED);
  ...
  if (currLaserH > 0) {
    ...
    for (double x = -0.1; x <= 0.1; x += resolution)
      for (double y = -0.1; y <= 0.1; y += resolution)
        maps3d[maps3dCnt - 1].SetOccupancyFromWorldFrame(
            pt(0) + x, pt(1) + y, pt(2), PROB_LASER_OCCUPIED);
  }
  // Floor Levels
  ...
  while (n.ok()) {
    ...
    if (cnt > 100) {
      ...
      for (int k = 0; k < maps3dCnt; k++) {
        msg.maps_active.push_back((bool)(!mapLinks[k](0)) &&
                                  mapLinks[k](2) > 50);
