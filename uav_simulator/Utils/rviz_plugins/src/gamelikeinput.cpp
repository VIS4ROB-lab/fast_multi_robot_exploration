...
GameLikeInput::GameLikeInput()
  ...
  topic_property_wp_ = new rviz::StringProperty(
      "TopicPoint", "point_list",
      "The topic on which to publish navigation goals.", getPropertyContainer(),
      SLOT(updateTopic()), this);

  topic_property_drone_ = new rviz::StringProperty(
      "TopicSelect", "select_list",
      "The topic on which to publish select drone id.", getPropertyContainer(),
      SLOT(updateTopic()), this);
  ...
  property_y_min =
      new rviz::FloatProperty("RangeY_min", -2.5, "", getPropertyContainer(),
                              SLOT(updateTopic()), this);
...
void GameLikeInput::onPoseSet(double x, double y, double z, double theta) {
  ...
  ROS_INFO(
      ...
      goal.pose.position.z, goal.pose.orientation.x, goal.pose.orientation.y,
