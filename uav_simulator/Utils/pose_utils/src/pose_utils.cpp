...
mat Jplus2(const colvec& X1, const colvec& X2) {
  ...
  double K2arr[9] = {
      ...
      cos(X3(6 - 1) - X2(6 - 1)),
      0,
      (a1(1 - 1) * cos(X3(4 - 1)) + a1(2 - 1) * sin(X3(4 - 1))) /
          cos(X3(5 - 1)),
...
mat jacobianF(const colvec& X, const colvec& U, double dt) {
  ...
  F(0, 3) = (dt * dt *
            ...
            2,
       F(0, 4) = (dt * dt *
                  (az * cos(pt) * cos(ro) * cos(ya) - ax * cos(ya) * sin(pt) +
  ...
  F(1, 3) = -(dt * dt *
            ...
            2;
  F(1, 4) = (dt * dt *
             (az * cos(pt) * cos(ro) * sin(ya) - ax * sin(pt) * sin(ya) +
  ...
  F(2, 3) = (dt * dt * (ay * cos(pt) * cos(ro) - az * cos(pt) * sin(ro))) / 2,
       F(2, 4) =
           -(dt * dt *
             (ax * cos(pt) + az * cos(ro) * sin(pt) + ay * sin(pt) * sin(ro))) /
           2,
       F(2, 5) = 0;
  F(2, 6) = 0;
  F(2, 7) = 0;
