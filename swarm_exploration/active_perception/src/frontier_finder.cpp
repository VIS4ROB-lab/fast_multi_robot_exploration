...
void FrontierFinder::searchFrontiers() {
  ...
  auto resetFlag = [&](list<Frontier>::iterator& iter, list<Frontier>& frontiers) {
    ...
    for (const auto& cell : iter->cells_) {
      edt_env_->sdf_map_->posToIndex(cell.second, idx);
...
bool FrontierFinder::splitHorizontally(const Frontier& frontier, list<Frontier>& splits) {
  ...
  for (const auto& cell : frontier.cells_) {
    if ((cell.second.head<2>() - mean).dot(first_pc) >= 0)
...
void FrontierFinder::computeFrontierInfo(Frontier& ftr) {
  ...
  ftr.box_max_ = ftr.cells_.front().second;
  ftr.box_min_ = ftr.cells_.front().second;
  for (const auto& cell : ftr.cells_) {
    auto cell_position = cell.second;
    ftr.average_ += cell_position;
    for (int i = 0; i < 3; ++i) {
      ftr.box_min_[i] = min(ftr.box_min_[i], cell_position[i]);
      ftr.box_max_[i] = max(ftr.box_max_[i], cell_position[i]);
    }
  }
...
void FrontierFinder::getTopViewpointsInfo(const Vector3d& cur_pos,
    vector<pair<LABEL, Eigen::Vector3d>>& points, vector<double>& yaws,
    vector<Eigen::Vector3d>& averages, vector<int>& ftr_ids) {
  points.clear();
  yaws.clear();
  averages.clear();
  for (const auto& frontier : frontiers_) {
    ...
    for (const auto& view : frontier.viewpoints_) {
      // Retrieve the first viewpoint that is far enough and has highest coverage
      if ((view.pos_ - cur_pos).norm() < min_candidate_dist_) continue;
      points.push_back({ frontier.label_, view.pos_ });
      yaws.push_back(view.yaw_);
      averages.push_back(frontier.average_);
      ftr_ids.push_back(frontier.id_);
      no_view = false;
      break;
    }
    if (no_view) {
...
void FrontierFinder::getFrontiers(vector<vector<Eigen::Vector3d>>& clusters) {
  ...
  for (const auto& frontier : frontiers_) {
    ...
    for (const auto& cell : frontier.cells_) cells.push_back(cell.second);
    clusters.push_back(cells);
  }
}

void FrontierFinder::getFrontiersIds(vector<int>& ids) {
  ids.clear();
  for (const auto& frontier : frontiers_) {
    ids.push_back(frontier.id_);
  }
}

void FrontierFinder::getLabeledFrontiers(
    vector<pair<LABEL, vector<Eigen::Vector3d>>>& labeled_clusters) {
  labeled_clusters.clear();
  for (const auto& frontier : frontiers_) {
    vector<Eigen::Vector3d> cells;
    for (const auto& cell : frontier.cells_) cells.push_back(cell.second);
    labeled_clusters.push_back({ frontier.label_, cells });
  }
}

void FrontierFinder::getTrailCentroidsAroundPosition(
    list<Frontier>& trails, const Eigen::Vector3d& position, double max_distance) const {
  trails.clear();
  for (const auto& frontier : frontiers_) {
    if (frontier.label_ == LABEL::TRAIL && (frontier.average_ - position).norm() <= max_distance) {
      trails.push_back(frontier);
    }
  }
}

void FrontierFinder::getInFrontFrontiers(
    vector<pair<LABEL, vector<Eigen::Vector3d>>>& infront_clusters) const {
  infront_clusters.clear();
  for (const auto& frontier : infront_frontiers_) {
    vector<Eigen::Vector3d> cells;
    for (const auto& cell : frontier.cells_) cells.push_back(cell.second);
    infront_clusters.push_back({ frontier.label_, cells });
  }
}
...
void FrontierFinder::getDormantFrontiers(vector<vector<Vector3d>>& clusters) {
  ...
  for (const auto& frontier : dormant_frontiers_) {
    vector<Eigen::Vector3d> dormant_cells;
    for (const auto& cell : frontier.cells_) dormant_cells.push_back(cell.second);
    clusters.push_back(dormant_cells);
  }
}
...
bool FrontierFinder::isFrontierInFront(const Eigen::Vector3d& ftr_pos, const Eigen::Vector3d& pos,
    const Eigen::Vector3d& vel, const double& yaw, double max_dist, double max_ang_dist) const {

  // TODO
  // It would make sense to check if we have a valid straight line path connecting the
  // drone to the frontiers to only have directly reachable frontiers

  const double min_norm_vel = 0.7;
  Eigen::Vector2d diff_pos = (ftr_pos - pos).head(2);
...
list<Frontier> FrontierFinder::updateFrontiersInFront(const Eigen::Vector3d& pos,
    const Eigen::Vector3d& vel, const double& yaw, double max_dist, double max_ang_dist) {
  infront_frontiers_.clear();
  for (const auto& ftr : frontiers_) {
    // Frontier survived checks - increment counter
    if (isFrontierInFront(ftr.average_, pos, vel, yaw, max_dist, max_ang_dist))
      infront_frontiers_.push_back(ftr);
  }
  return infront_frontiers_;
}
...
int FrontierFinder::deleteFrontiers(const vector<uint16_t>& ids) {
  ...
  auto tmp_ids = ids;
  std::sort(tmp_ids.begin(), tmp_ids.end(), greater<int>());
...
void FrontierFinder::findViewpoints(
  ...
  vps.clear();
  std::sort(yaw_gains.begin(), yaw_gains.end(),
...
void FrontierFinder::downsample(
    const vector<pair<LABEL, Eigen::Vector3d>>& cluster_in, vector<Eigen::Vector3d>& cluster_out) {
  // downsamping cluster
  pcl::PointCloud<pcl::PointXYZ>::Ptr cloud(new pcl::PointCloud<pcl::PointXYZ>);
  pcl::PointCloud<pcl::PointXYZ>::Ptr cloudf(new pcl::PointCloud<pcl::PointXYZ>);
  for (const auto& cell : cluster_in) {
    auto cell_position = cell.second;
    cloud->points.emplace_back(cell_position[0], cell_position[1], cell_position[2]);
  }
...
void FrontierFinder::binaryClassify() {
  // Iterate over clusters. A cluster is classified as TRAIL if:
  // 1. it has only 1 neighbouring cluster, or
  // 2. it surrounded by free space (i.e. number of neighbours is 0)

  // Database with number of neighbours per frontier and ids (ID - list of neighbours frontiers IDs)
  std::unordered_map<int, std::vector<int>> neighbours_database;

  auto countNeighbours = [&](const Frontier& ftr_query, std::vector<int>& neigh_ids) {
    // Parameters
    const double kMinDist = 0.25 * cluster_size_xy_;
    const double kSearchRadius = 5.0 * cluster_size_xy_;

    // Query bounding box
    const auto& query_max = ftr_query.box_max_;
    const auto& query_min = ftr_query.box_min_;
    const Eigen::Vector2d p1(query_max.x(), query_max.y());
    const Eigen::Vector2d p2(query_max.x(), query_min.y());
    const Eigen::Vector2d p3(query_min.x(), query_max.y());
    const Eigen::Vector2d p4(query_min.x(), query_min.y());

    for (const auto& ftr_candidate : frontiers_) {
      // It's the same frontier, so skip it
      if (ftr_candidate.id_ == ftr_query.id_) {
        continue;
      }

      // Search for frontiers only within a radius
      if ((ftr_candidate.average_ - ftr_query.average_).head(2).norm() >= kSearchRadius) {
        continue;
      }

      // Check the bounding boxes
      const auto& candidate_max = ftr_candidate.box_max_;
      const auto& candidate_min = ftr_candidate.box_min_;
      const Eigen::Vector2d c1(candidate_max.x(), candidate_max.y());
      const Eigen::Vector2d c2(candidate_max.x(), candidate_min.y());
      const Eigen::Vector2d c3(candidate_min.x(), candidate_max.y());
      const Eigen::Vector2d c4(candidate_min.x(), candidate_min.y());

      if ((p1 - c1).norm() <= kMinDist)
        neigh_ids.push_back(ftr_candidate.id_);
      else if ((p1 - c2).norm() <= kMinDist)
        neigh_ids.push_back(ftr_candidate.id_);
      else if ((p1 - c3).norm() <= kMinDist)
        neigh_ids.push_back(ftr_candidate.id_);
      else if ((p1 - c4).norm() <= kMinDist)
        neigh_ids.push_back(ftr_candidate.id_);
      //
      else if ((p2 - c1).norm() <= kMinDist)
        neigh_ids.push_back(ftr_candidate.id_);
      else if ((p2 - c2).norm() <= kMinDist)
        neigh_ids.push_back(ftr_candidate.id_);
      else if ((p2 - c3).norm() <= kMinDist)
        neigh_ids.push_back(ftr_candidate.id_);
      else if ((p2 - c4).norm() <= kMinDist)
        neigh_ids.push_back(ftr_candidate.id_);
      //
      else if ((p3 - c1).norm() <= kMinDist)
        neigh_ids.push_back(ftr_candidate.id_);
      else if ((p3 - c2).norm() <= kMinDist)
        neigh_ids.push_back(ftr_candidate.id_);
      else if ((p3 - c3).norm() <= kMinDist)
        neigh_ids.push_back(ftr_candidate.id_);
      else if ((p3 - c4).norm() <= kMinDist)
        neigh_ids.push_back(ftr_candidate.id_);
      //
      else if ((p4 - c1).norm() <= kMinDist)
        neigh_ids.push_back(ftr_candidate.id_);
      else if ((p4 - c2).norm() <= kMinDist)
        neigh_ids.push_back(ftr_candidate.id_);
      else if ((p4 - c3).norm() <= kMinDist)
        neigh_ids.push_back(ftr_candidate.id_);
      else if ((p4 - c4).norm() <= kMinDist)
        neigh_ids.push_back(ftr_candidate.id_);
    }
  };

  for (auto& ftr : frontiers_) {
    // Count the number of neighbouring clusters and get their IDs
    neighbours_database[ftr.id_] = {};
    countNeighbours(ftr, neighbours_database[ftr.id_]);
  }
  ...
  for (auto& ftr : frontiers_) {
    ...
    // Get IDs of neighbouring frontiers and assign label
    if (neighbours_database[ftr.id_].empty()) {
      // If there are no neighbours, then it's a trail
      label = LABEL::TRAIL;
    } else if (neighbours_database[ftr.id_].size() >= 2) {
      // If we have at least 2 neighbours, then it's a frontier
      label = LABEL::FRONTIER;
    } else {
      // Ambigous case: we have only 1 neighbours, but then:
      // - it will be a FRONTIER if its neighbour has another neighbour
      // - it will be a TRAIL if  its neighbour does not have another neighbour
      int neigh_id = neighbours_database[ftr.id_][0];
      size_t num_neigh = neighbours_database[neigh_id].size() - 1;  // Don't cound ftr.id_ twice
      label = num_neigh == 0 ? LABEL::TRAIL : LABEL::FRONTIER;
    }

    ftr.label_ = label;
    for (auto& cell : ftr.cells_) cell.first = label;
  }
}
