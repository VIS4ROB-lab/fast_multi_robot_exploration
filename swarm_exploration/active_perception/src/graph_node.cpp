...
bool ViewNode::straightLineFeasible(const Vector3d& p1, const Vector3d& p2) {
  Vector3i idx;
  caster_->input(p1, p2);
  while (caster_->nextId(idx)) {
    if (map_->getInflateOccupancy(idx) == 1 || map_->getOccupancy(idx) != SDFMap::FREE ||
        !map_->isInBox(idx)) {
      return false;
    }
  }
...
bool ViewNode::validPathExists(const Vector3d& p1, const Vector3d& p2) {
  // Try connect two points with straight line
  if (straightLineFeasible(p1, p2)) {
    return true;
  }
...
bool ViewNode::validPathExists(const Vector3d& p1, const Vector3d& p2, double& path_length) {
  // Try connect two points with straight line
  if (straightLineFeasible(p1, p2)) {
    return true;
  }
  ...
  for (size_t k = 0; k < res.size(); ++k) {
    ...
    if (astar_->search(p1, p2) == Astar::REACH_END) {
      ...
      path_length = astar_->pathLength(path);
      return true;
    }
  }
  // No path found
