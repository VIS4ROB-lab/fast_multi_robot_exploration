...
void FastExplorationFSM::init(ros::NodeHandle& nh) {
  ...
  heartbit_timer_ = nh.createTimer(ros::Duration(1.0), &FastExplorationFSM::heartbitCallback, this);
...
void FastExplorationFSM::sendEmergencyMsg(bool emergency) {
  std_msgs::Bool msg;
  msg.data = emergency;
  emergency_handler_pub_.publish(msg);
}

bool FastExplorationFSM::isExperimentDone(const Eigen::Vector3d& pos) {
  if (state_ == WAIT_TRIGGER) return false;

  static auto check_time = ros::Time::now();
  if ((ros::Time::now() - check_time).toSec() < 30.0) {
    return false;
  }

  // Check if we have moved
  const double kMinDist = 0.02;
  bool finished_exp = (pos - fd_->odom_pos_).norm() < kMinDist;
...
void FastExplorationFSM::FSMCallback(const ros::TimerEvent& e) {
  ...
  switch (state_) {
    ...
    case IDLE: {
      ...
      if (expl_manager_->updateFrontierStruct(fd_->odom_pos_) <= 1) {
...
void FastExplorationFSM::heartbitCallback(const ros::TimerEvent& e) {
...
void FastExplorationFSM::odometryCallback(const nav_msgs::OdometryConstPtr& msg) {
  ...
  // );
  // if (isExperimentDone(pos)) {
  //   sendStopMsg(1);
  // }

  // Else: experiment not done
