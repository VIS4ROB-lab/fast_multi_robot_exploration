...
public:
  ...
  double getKinodynamicAstarAMax() const {
    return kino_path_finder_->getAMax();
  }

  void setKinodynamicAstarAMax(double max_acc) {
    kino_path_finder_->setAMax(max_acc);
  }

  double getMaxVelBsplineOpt() const {
    double max_vel = std::numeric_limits<double>::min();
    for (const auto& opt : bspline_optimizers_) max_vel = max(max_vel, opt->getMaxVel());
    return max_vel;
  }

  double getMaxAccBsplineOpt() const {
    double max_acc = std::numeric_limits<double>::min();
    for (const auto& opt : bspline_optimizers_) max_acc = max(max_acc, opt->getMaxAcc());
    return max_acc;
  }

  void setMaxVelBsplineOpt(double max_vel) {
    for (auto& opt : bspline_optimizers_) opt->setMaxVel(max_vel);
  }

  void setMaxAccBsplineOpt(double max_acc) {
    for (auto& opt : bspline_optimizers_) opt->setMaxAcc(max_acc);
  }
