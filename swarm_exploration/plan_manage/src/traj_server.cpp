...
ros::Publisher cmd_vis_pub, pos_cmd_pub, traj_pub, path_pub, swarm_pos_cmd_pub;
...
std::vector<Eigen::Vector3d> prev_odom;
...
void displayTrajWithColor(
  ...
  path_msg.header = mk.header;

  geometry_msgs::PoseStamped pose_stamped_msg;
  pose_stamped_msg.header.frame_id = mk.header.frame_id;
  ...
  for (const auto& p : path) {
    pose_stamped_msg.pose.position.x = p[0];
    pose_stamped_msg.pose.position.y = p[1];
    pose_stamped_msg.pose.position.z = p[2];
...
void triggerCallback(const std_msgs::BoolConstPtr& msg) {
  // If it was an emergency, and not it is not anymore, clear buffer
...
void cmdCallback(const ros::TimerEvent& e) {
  ...
  if (emergency_) {
    ...
    cmd.trajectory_id = 4;
    cmd.position.x = prev_odom.back().x();
    cmd.position.y = prev_odom.back().y();
    cmd.position.z = prev_odom.back().z();
    cmd.velocity.x = 0.;
    cmd.velocity.y = 0.;
    cmd.velocity.z = 0.;
...
int main(int argc, char** argv) {
  ...
  ros::Subscriber trigger_sub = node.subscribe("/trigger_emergency", 1, triggerCallback);
  ...
  path_pub = node.advertise<nav_msgs::Path>("planning/travel_path", 10);
  // swarm_pos_cmd_pub =
