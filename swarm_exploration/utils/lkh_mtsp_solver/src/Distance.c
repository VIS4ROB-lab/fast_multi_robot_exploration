...
int Distance_Asymmetric(Node* Na, Node* Nb) {
  ...
  if ((Na->Id <= n) == (Nb->Id <= n)) return M;
  if (abs(Na->Id - Nb->Id) == n) return 0;
...
int Distance_GEO(Node* Na, Node* Nb) {
  int deg;
  double NaLatitude, NaLongitude, NbLatitude, NbLongitude, min, q1, q2, q3;
  deg = (int)Na->X;
  min = Na->X - deg;
...
int Distance_MAN_2D(Node* Na, Node* Nb) {
  return (int)(Scale * (fabs(Na->X - Nb->X) + fabs(Na->Y - Nb->Y)) + 0.5);
}

int Distance_MAN_3D(Node* Na, Node* Nb) {
  return (int)(Scale * (fabs(Na->X - Nb->X) + fabs(Na->Y - Nb->Y) + fabs(Na->Z - Nb->Z)) + 0.5);
}
...
int Distance_GEO_MEEUS(Node* Na, Node* Nb) {
  double lat1 = M_PI * ((int)Na->X + 5 * (Na->X - (int)Na->X) / 3) / 180;
  double lon1 = M_PI * ((int)Na->Y + 5 * (Na->Y - (int)Na->Y) / 3) / 180;
  double lat2 = M_PI * ((int)Nb->X + 5 * (Nb->X - (int)Nb->X) / 3) / 180;
  double lon2 = M_PI * ((int)Nb->Y + 5 * (Nb->Y - (int)Nb->Y) / 3) / 180;
