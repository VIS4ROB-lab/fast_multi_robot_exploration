...
Node* Best2OptMove(Node* t1, Node* t2, GainType* G0, GainType* Gain) {
  ...
  for (Nt2 = t2->CandidateSet; (t3 = Nt2->To); Nt2++) {
    ...
    G2 = G1 + C(t3, t4);
    if (!Forbidden(t4, t1) && (CurrentPenalty != 0 || TSPTW_Makespan || !c || G2 - c(t4, t1) > 0)) {
      *Gain = G2 - C(t4, t1);
    ...
    if (GainCriterionUsed && G2 - Precision < t4->Cost) continue;
    if (!Backtracking || Swaps > 0) {
      if ((G2 > BestG2 || (G2 == BestG2 && !Near(t3, t4) && Near(T3, T4))) && Swaps < MaxSwaps &&
