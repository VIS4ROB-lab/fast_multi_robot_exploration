...
void AddToPopulation(GainType Penalty, GainType Cost) {
  ...
  if (!Population) {
    ...
    PenaltyFitness = (GainType*)malloc(MaxPopulationSize * sizeof(GainType));
    Fitness = (GainType*)malloc(MaxPopulationSize * sizeof(GainType));
  }
  for (i = PopulationSize; i >= 1 && SmallerFitness(Penalty, Cost, i - 1); i--) {
...
void ApplyCrossover(int i, int j) {
  ...
  if (TraceLevel >= 1) printff("Crossover(%d,%d)\n", i + 1, j + 1);
  /* Apply the crossover operator */
  Crossover();
  if (ProblemType == SOP) SOP_RepairTour();
}
...
void FreePopulation() {
  if (Population) {
    int i;
    for (i = 0; i < MaxPopulationSize; i++) Free(Population[i]);
    Free(Population);
    Free(PenaltyFitness);
    Free(Fitness);
  }
...
int HasFitness(GainType Penalty, GainType Cost) {
  ...
  return High >= 0 && PenaltyFitness[High] == Penalty && Fitness[High] == Cost;
}
...
int LinearSelection(int Size, double Bias) {
  return (int)(Size * (Bias - sqrt((Bias * Bias - 4 * (Bias - 1) * Random01()))) / 2 / (Bias - 1));
}
...
GainType MergeTourWithIndividual(int i) {
  ...
  Pi = Population[i];
  for (k = 1; k <= Dimension; k++) NodeSet[Pi[k - 1]].Next = &NodeSet[Pi[k]];
...
void PrintPopulation() {
  ...
  for (i = 0; i < PopulationSize; i++) {
    printff("%3d: ", i + 1);
    if (Penalty)
      printff(GainFormat "_" GainFormat, PenaltyFitness[i], Fitness[i]);
    else
    ...
    if (Optimum != MINUS_INFINITY && Optimum != 0) {
      if (ProblemType != CCVRP && ProblemType != TRP && ProblemType != MLP &&
        ...
        printff(", Gap = %0.4f%%", 100.0 * (Fitness[i] - Optimum) / Optimum);
      else
        printff(", Gap = %0.4f%%", 100.0 * (PenaltyFitness[i] - Optimum) / Optimum);
    }
    printff("\n");
  }
}
...
static int DistanceToIndividual(int i) {
  ...
  N = FirstNode;
  do
    if (N->OldSuc != N->Next && N->OldSuc != N->Prev) Count++;
...
int ReplacementIndividual(GainType Penalty, GainType Cost) {
  ...
  while ((N = N->OldSuc = N->Suc) != FirstNode)
    ;
  for (i = PopulationSize - 1; i >= 0 && SmallerFitness(Penalty, Cost, i); i--) {
    if ((d = DistanceToIndividual(i)) < MinDist) {
      CMin = i;
      MinDist = d;
    }
  }
  if (CMin == PopulationSize - 1) return CMin;
  P = Population[CMin];
  for (j = 0; j < Dimension; j++) NodeSet[P[j]].OldSuc = &NodeSet[P[j + 1]];
  for (i = 0; i < PopulationSize; i++)
    if (i != CMin && (d = DistanceToIndividual(i)) <= MinDist) return PopulationSize - 1;
  return CMin;
}
