...
void GenerateCandidates(int MaxCandidates, GainType MaxAlpha, int Symmetric) {
  ...
  if (TraceLevel >= 2) printff("Generating candidates ... ");
  ...
  if (MaxCandidates > 0) {
    do {
      From->CandidateSet = (Candidate*)malloc((MaxCandidates + 1) * sizeof(Candidate));
  ...
  do {
    ...
    do {
      ...
      if (FixedOrCommon(From, To))
        ...
        if (From->FixedTo2 || To->FixedTo2 || Forbidden(From, To)) continue;
        if (InInputTour(From, To)) {
          ...
          if (a > MaxAlpha ||
              (Count == MaxCandidates &&
                  (a > (NFrom - 1)->Alpha || (a == (NFrom - 1)->Alpha && d >= (NFrom - 1)->Cost))))
            continue;
          if (To == From->Dad) {
            d = From->Cost;
            a = 0;
      ...
      if (a <= MaxAlpha && IsPossibleCandidate(From, To)) {
        ...
        while (--NN >= From->CandidateSet) {
          if (a > NN->Alpha || (a == NN->Alpha && d >= NN->Cost)) break;
          *(NN + 1) = *NN;
        }
        NN++;
        NN->To = To;
