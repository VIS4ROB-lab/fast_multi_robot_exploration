...
Node** BuildKDTree(int Cutoff) {
  ...
  for (i = 0, N = FirstNode; i < Dimension; i++, N = N->Suc) KDTree[i] = N;
...
static void Partition(int start, int end, int k, int axis) {
  while (start < end) {
    ...
    double pivot;
    if (Coord(KDTree[mid], axis) < Coord(KDTree[start], axis)) Swap(start, mid);
    if (Coord(KDTree[end], axis) < Coord(KDTree[start], axis)) Swap(start, end);
    if (Coord(KDTree[end], axis) < Coord(KDTree[mid], axis)) Swap(mid, end);
