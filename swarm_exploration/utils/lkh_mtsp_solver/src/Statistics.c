...
void UpdateStatistics(GainType Cost, double Time) {
  ...
  if (ProblemType != CCVRP && ProblemType != TRP && ProblemType != MLP && MTSPObjective != MINMAX &&
    ...
    if (CurrentPenalty == 0 && Cost <= Optimum) Successes++;
  } else if (CurrentPenalty <= Optimum)
    Successes++;
  if (CurrentPenalty < PenaltyMin) PenaltyMin = CurrentPenalty;
  if (CurrentPenalty > PenaltyMax) PenaltyMax = CurrentPenalty;
...
void PrintStatistics() {
  ...
  if (ProblemType != CCVRP && ProblemType != TRP && ProblemType != MLP && MTSPObjective != MINMAX &&
      MTSPObjective != MINMAX_SIZE && CostMin <= CostMax && CostMin != PLUS_INFINITY) {
    ...
    printff("Penalty.min = " GainFormat ", Penalty.avg = %0.2f, "
            "Penalty.max = " GainFormat "\n",
        PenaltyMin, (double)PenaltySum / _Runs, PenaltyMax);
    if (_Optimum == MINUS_INFINITY) _Optimum = BestPenalty;
    if (_Optimum != 0)
      printff("Gap.min = %0.4f%%, Gap.avg = %0.4f%%, "
              "Gap.max = %0.4f%%\n",
  ...
  printff("Trials.min = %d, Trials.avg = %0.1f, Trials.max = %d\n", _TrialsMin,
      1.0 * TrialSum / _Runs, TrialsMax);
  printff("Time.min = %0.2f sec., Time.avg = %0.2f sec., Time.max = %0.2f sec.\n", fabs(_TimeMin),
      fabs(TimeSum) / _Runs, fabs(TimeMax));
}
