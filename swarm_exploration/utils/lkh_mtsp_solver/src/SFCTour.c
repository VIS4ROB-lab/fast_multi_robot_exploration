...
GainType SFCTour(int CurveType) {
  ...
  if (CurveType == SIERPINSKI) {
    if (TraceLevel >= 1) printff("Sierpinski = ");
    Index = SierpinskiIndex;
  } else {
    if (TraceLevel >= 1) printff("Moore = ");
    Index = MooreIndex;
  }
  N = FirstNode;
  XMin = XMax = N->X;
  ...
  for (i = 0, N = FirstNode; i < Dimension; i++, N = N->Suc)
    (Perm[i] = N)->V = Index((N->X - XMin) / (XMax - XMin), (N->Y - YMin) / (YMax - YMin));
  qsort(Perm, Dimension, sizeof(Node*), compare);
  for (i = 1; i < Dimension; i++) Follow(Perm[i], Perm[i - 1]);
  free(Perm);
  ...
  N = FirstNode;
  do
    if (!Fixed(N, N->Suc)) Cost += Distance(N, N->Suc);
  while ((N = N->Suc) != FirstNode);
...
static int MooreIndex(double x, double y) {
  static const int Rank[5][4] = { { 1, 0, 2, 3 }, { 2, 3, 1, 0 }, { 2, 1, 3, 0 }, { 0, 3, 1, 2 },
    { 0, 1, 3, 2 } };
  static const int NextState[5][4] = { { 2, 2, 3, 3 }, { 1, 3, 1, 2 }, { 2, 2, 4, 1 },
    { 4, 1, 3, 3 }, { 3, 4, 2, 4 } };
  int idx = 0, State = 0;
  int i = 4 * sizeof(int);
