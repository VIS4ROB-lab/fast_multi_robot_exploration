...
int C_FUNCTION(Node* Na, Node* Nb) {
  ...
  if (CostMatrix) return D(Na, Nb);
  if (PredSucCostAvailable) {
    if (Na->Suc == Nb) return Na->SucCost;
    if (Na->Pred == Nb) return Na->PredCost;
  }
  if ((Cand = Na->CandidateSet))
    for (; (Nc = Cand->To); Cand++)
      if (Nc == Nb) return Cand->Cost;
  if ((Cand = Nb->CandidateSet))
    for (; (Nc = Cand->To); Cand++)
      if (Nc == Na) return Cand->Cost;
  ...
  if (CacheSig[Index] == i) return CacheVal[Index];
  CacheSig[Index] = i;
...
int D_FUNCTION(Node* Na, Node* Nb) {
  return (Fixed(Na, Nb) ? 0 : Distance(Na, Nb) * Precision) + Na->Pi + Nb->Pi;
}

/* Functions for computing lower bounds for the distance functions */

int c_ATT(Node* Na, Node* Nb) {
  if (Fixed(Na, Nb)) return Na->Pi + Nb->Pi;
...
int c_CEIL_2D(Node* Na, Node* Nb) {
  if (Fixed(Na, Nb)) return Na->Pi + Nb->Pi;
  int dx = (int)ceil(Scale * fabs(Na->X - Nb->X)), dy = (int)ceil(Scale * fabs(Na->Y - Nb->Y));
  return (dx > dy ? dx : dy) * Precision + Na->Pi + Nb->Pi;
}
...
int c_EUC_2D(Node* Na, Node* Nb) {
  if (Fixed(Na, Nb)) return Na->Pi + Nb->Pi;
  int dx = (int)(Scale * fabs(Na->X - Nb->X) + 0.5), dy = (int)(Scale * fabs(Na->Y - Nb->Y) + 0.5);
  return (dx > dy ? dx : dy) * Precision + Na->Pi + Nb->Pi;
}
...
int c_FLOOR_2D(Node* Na, Node* Nb) {
  if (Fixed(Na, Nb)) return Na->Pi + Nb->Pi;
  int dx = (int)floor(Scale * fabs(Na->X - Nb->X)), dy = (int)floor(Scale * fabs(Na->Y - Nb->Y));
  return (dx > dy ? dx : dy) * Precision + Na->Pi + Nb->Pi;
}
...
int c_GEO(Node* Na, Node* Nb) {
  ...
  int dx = (int)(RRR * PI / 180.0 * fabs(da - db + 5.0 * (ma - mb) / 3.0) + 1.0);
  return dx * Precision + Na->Pi + Nb->Pi;
}
...
int c_GEO_MEEUS(Node* Na, Node* Nb) {
  ...
  int da = (int)Na->X, db = (int)Nb->X;
  double ma = Na->X - da, mb = Nb->X - db;
  int dx = (int)(RRR * M_PI / 180.0 * fabs(da - db + 5.0 * (ma - mb) / 3.0) * f + 0.5);
  return dx * Precision + Na->Pi + Nb->Pi;
}
