...
static void FlipSegments(Segment* a, Segment* b, Segment* c, Segment* d);
static void FlipSSegments(SSegment* a, SSegment* b, SSegment* c, SSegment* d);
...
void Flip_SSL(Node* t1, Node* t2, Node* t3) {
  ...
  } else if ((P1 == P3 && abs(t3->Rank - t1->Rank) > SPLIT_CUTOFF * GroupSize) ||
             (P2 == P4 && abs(t4->Rank - t2->Rank) > SPLIT_CUTOFF * GroupSize)) {
    if (P1 == P2) {
      SplitSegment(t1, t2);
      P1 = t1->Parent;
      P2 = t2->Parent;
  ...
  if (b) /* Flip locally (b --> d) within a segment */
    FlipNodes(a, b, c, d);
  else {
    ...
    } else if ((P1 == P3 && abs(t3->Rank - t1->Rank) > SPLIT_CUTOFF * SGroupSize) ||
               (P2 == P4 && abs(t4->Rank - t2->Rank) > SPLIT_CUTOFF * SGroupSize)) {
      if (P1 == P2) {
        SplitSSegment(t1, t2);
        P1 = t1->Parent;
        P2 = t2->Parent;
    ...
    if (P1 == P3) {
      if (t1->Rank < t3->Rank) {
        ...
        if (P1 == P2 && P1 == P4 && t2->Rank < t1->Rank && t1->Rank - t2->Rank + 1 < Groups) {
          a = t3;
          b = t4;
          c = t1;
          d = t2;
    ...
    if (b) /* Flip locally (b --> d) within a super segment */
      ...
      if ((i = P2->Rank - P3->Rank) <= 0) i += SGroups;
      if (2 * i > SGroups) {
        a = t3;
        t3 = t2;
        t2 = a;
