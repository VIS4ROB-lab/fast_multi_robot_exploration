  ...
  while ((Line = ReadLine(ParameterFile))) {
    ...
    if (!strcmp(Keyword, "ASCENT_CANDIDATES")) {
      ...
      if (CandidateFiles == 0) {
        ...
        if (i == CandidateFiles) {
          CandidateFileName =
              (char**)realloc(CandidateFileName, (CandidateFiles + 1) * sizeof(char*));
          CandidateFileName[CandidateFiles++] = Name;
        }
      }
      ...
      for (i = 0; i < strlen(Token); i++) Token[i] = (char)toupper(Token[i]);
      ...
      if (MergeTourFiles == 0) {
        ...
        if (i == MergeTourFiles) {
          MergeTourFileName =
              (char**)realloc(MergeTourFileName, (MergeTourFiles + 1) * sizeof(char*));
          MergeTourFileName[MergeTourFiles++] = Name;
        }
      }
    } else if (!strcmp(Keyword, "MOVE_TYPE")) {
      if (!(Token = strtok(0, Delimiters)) || !sscanf(Token, "%d", &MoveType))
      ...
      if ((Token = strtok(0, Delimiters))) {
        ...
        if (!strncmp(Token, "DELAUNAY", strlen(Token)))
          ...
          eprintf("(SUBPROBLEM_SIZE) Illegal DELAUNAY, KARP, K-CENTER, "
                  "K-MEANS, MOORE, ROHE,\n SIERPINSKI, "
        ...
        while ((Token = strtok(0, Delimiters))) {
          ...
          if (!strncmp(Token, "BORDERS", strlen(Token)))
            ...
            eprintf("(SUBPROBLEM_SIZE) Illegal BORDERS or "
...
static char* GetFileName(char* Line) {
  ...
  while (isspace(*Rest)) Rest++;
  for (t = Rest + strlen(Rest) - 1; isspace(*t); t--) *t = '\0';
