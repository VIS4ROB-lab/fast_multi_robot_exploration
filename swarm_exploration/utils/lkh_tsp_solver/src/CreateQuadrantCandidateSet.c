...
void CreateQuadrantCandidateSet(int K) {
  ...
  if (K <= 0) return;
  if (TraceLevel >= 2) printff("Creating quadrant candidate set ... ");
  ...
  BoxOverlaps = CoordType == THREED_COORDS ? BoxOverlaps3D : BoxOverlaps2D;
  ...
  do {
    ...
    for (NFrom = From->CandidateSet; NFrom && NFrom->To; NFrom++)
      if (FixedOrCommon(From, NFrom->To) && ++Count == 2) break;
    if (Count == 2) continue;
    Added = 0;
    for (Q = 1; Q <= L; Q++) {
      ...
      for (i = 0; i < Candidates; i++) {
        ...
        if (AddCandidate(From, To, D(From, To), 1)) Added++;
      }
    }
    if (K > Added) {
  ...
  if (Level == 0 && (WeightType == GEO || WeightType == GEOM || WeightType == GEO_MEEUS ||
                        WeightType == GEOM_MEEUS)) {
    Candidate** SavedCandidateSet;
    SavedCandidateSet = (Candidate**)malloc((1 + DimensionSaved) * sizeof(Candidate*));
    if (TraceLevel >= 2)
      ;
    From = FirstNode;
    while ((From = From->Suc) != FirstNode)
      if ((From->Y > 0) != (FirstNode->Y > 0)) break;
    if (From != FirstNode) {
      ...
      do {
        ...
        if (WeightType == GEO || WeightType == GEO_MEEUS)
          From->Y = (int)From->Y + 5.0 * (From->Y - (int)From->Y) / 3.0;
        From->Y += From->Y > 0 ? -180 : 180;
        if (WeightType == GEO || WeightType == GEO_MEEUS)
          From->Y = (int)From->Y + 3.0 * (From->Y - (int)From->Y) / 5.0;
      } while ((From = From->Suc) != FirstNode);
      Level++;
...
void CreateNearestNeighborCandidateSet(int K) {
  ...
  if (TraceLevel >= 2) printff("Creating nearest neighbor candidate set ... ");
  KDTree = BuildKDTree(1);
  XMin = (double*)malloc((1 + DimensionSaved) * sizeof(double));
  XMax = (double*)malloc((1 + DimensionSaved) * sizeof(double));
  YMin = (double*)malloc((1 + DimensionSaved) * sizeof(double));
...
static void ComputeBounds(int start, int end) {
  if (start <= end) {
    ...
    for (i = start; i <= end; i++) {
      Node* N = KDTree[i];
      if (N == T) continue;
      if (N->X < XMin[T->Id]) XMin[T->Id] = N->X;
      if (N->X > XMax[T->Id]) XMax[T->Id] = N->X;
      if (N->Y < YMin[T->Id]) YMin[T->Id] = N->Y;
      if (N->Y > YMax[T->Id]) YMax[T->Id] = N->Y;
      if (CoordType == THREED_COORDS) {
        if (N->Z < ZMin[T->Id]) ZMin[T->Id] = N->Z;
        if (N->Z > ZMax[T->Id]) ZMax[T->Id] = N->Z;
      }
    }
...
static int BoxOverlaps3D(Node* T, int Q, Node* N) {
  switch (Q) {
    case 1:
      return XMax[T->Id] >= N->X && YMax[T->Id] >= N->Y && ZMax[T->Id] >= N->Z;
    case 2:
      return XMin[T->Id] <= N->X && YMax[T->Id] >= N->Y && ZMax[T->Id] >= N->Z;
    case 3:
      return XMin[T->Id] <= N->X && YMin[T->Id] <= N->Y && ZMax[T->Id] >= N->Z;
    case 4:
      return XMax[T->Id] >= N->X && YMin[T->Id] <= N->Y && ZMax[T->Id] >= N->Z;
    case 5:
      return XMax[T->Id] >= N->X && YMax[T->Id] >= N->Y && ZMin[T->Id] <= N->Z;
    case 6:
      return XMin[T->Id] <= N->X && YMax[T->Id] >= N->Y && ZMin[T->Id] <= N->Z;
    case 7:
      return XMin[T->Id] <= N->X && YMin[T->Id] <= N->Y && ZMin[T->Id] <= N->Z;
    case 8:
      return XMax[T->Id] >= N->X && YMin[T->Id] <= N->Y && ZMin[T->Id] <= N->Z;
    default:
      return 1;
  }
}
...
static int InCandidateSet(Node* N, Node* T) {
  int i;
  for (i = 0; i < Candidates; i++)
    if (CandidateSet[i].To == T) return 1;
...
static void NQN(Node* N, int Q, int start, int end, int K) {
  ...
  if (K == 0 || N->FixedTo2) return;
  if (start <= end && T != N && !T->FixedTo2 && IsPossibleCandidate(N, T) && Contains(T, Q, N) &&
      !InCandidateSet(N, T) && (!c || c(N, T) - N->Pi - T->Pi <= Radius) &&
      (d = Distance(N, T) * Precision) <= Radius) {
    int i = Candidates;
    while (--i >= 0 && d < CandidateSet[i].Cost) CandidateSet[i + 1] = CandidateSet[i];
    CandidateSet[i + 1].To = T;
    CandidateSet[i + 1].Cost = d;
    if (Candidates < K) Candidates++;
    if (Candidates == K) Radius = CandidateSet[Candidates - 1].Cost;
  }
  if (start < end && BoxOverlaps(T, Q, N)) {
    ...
    P.Pi = 0;
    if (diff >= 0) {
      if (Overlaps(Q, diff, 0, axis)) NQN(N, Q, start, mid - 1, K);
      if (Overlaps(Q, diff, 1, axis) && (!c || c(N, &P) - N->Pi <= Radius) &&
          Distance(N, &P) * Precision <= Radius)
        NQN(N, Q, mid + 1, end, K);
    } else {
      if (Overlaps(Q, diff, 1, axis)) NQN(N, Q, mid + 1, end, K);
      if (Overlaps(Q, diff, 0, axis) && (!c || c(N, &P) - N->Pi <= Radius) &&
          Distance(N, &P) * Precision <= Radius)
