...
GainType MergeWithTourGPX2() {
  ...
  Cost1 /= Precision;
  if (NewDimension == 0) return Cost1;
  ...
  do {
    if (N->V) {
      if (Last) (Last->OldSuc = N)->OldPred = Last;
      Last = N;
    }
  } while ((N = N->Suc) != First);
  ...
  do {
    if (N->V) {
      if (Last) {
        Last->Next = N;
        if (Last != N->Prev) N->Prev = Last;
      }
      Last = N;
    }
  } while ((N = N->Next) != First);
  Last->Next = First;
  if (Last != First->Prev) First->Prev = Last;
  ...
  Cost1 /= Precision;
  if (TraceLevel >= 2) printff("GPX2: " GainFormat "\n", Cost1);
  return Cost1;
}
