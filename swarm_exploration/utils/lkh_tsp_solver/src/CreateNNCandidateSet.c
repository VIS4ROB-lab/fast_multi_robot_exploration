...
void CreateNNCandidateSet(int K) {
  ...
  if (TraceLevel >= 2) printff("Creating NN candidate set ... \n");
  XNearList = (Node**)malloc(Dimension * sizeof(Node*));
  To = (Node**)malloc((K + 1) * sizeof(Node*));
  Cost = (int*)malloc((K + 1) * sizeof(int));
  for (Na = FirstNode, k = 0; k < Dimension; Na = Na->Suc, k++) XNearList[k] = Na;
  ...
  for (a = 0; a < Dimension; a++) {
    ...
    for (k = 0; k < Count; k++) AddCandidate(Na, To[k], D(Na, To[k]), 2);
  }
  free(Cost);
  free(To);
  free(XNearList);
