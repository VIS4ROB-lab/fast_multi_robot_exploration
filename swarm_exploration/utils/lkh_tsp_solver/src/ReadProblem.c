...
void ReadProblem() {
  ...
  if (CostMatrix == 0 && Dimension <= MaxMatrixDimension && Distance != 0 &&
      Distance != Distance_1 && Distance != Distance_LARGE && Distance != Distance_ATSP &&
...
static void CheckSpecificationPart() {
  ...
  if (WeightType == -1 && ProblemType != ATSP && ProblemType != HCP && ProblemType != HPP &&
      !EdgeWeightType)
  ...
  if (ExtraCandidateSetType == NN && !TwoDWeightType() && !ThreeDWeightType() &&
      ExtraCandidates > 0)
...
static void Read_DISPLAY_DATA_SECTION() {
  ...
  for (i = 1; i <= Dimension; i++) {
    ...
    N->V = 1;
    if (!fscanf(ProblemFile, "%lf", &N->X)) eprintf("DISPLAY_DATA_SECTION: Missing X-coordinate");
    if (!fscanf(ProblemFile, "%lf", &N->Y)) eprintf("DISPLAY_DATA_SECTION: Missing Y-coordinate");
...
static void Read_EDGE_DATA_SECTION() {
  ...
  if (!strcmp(EdgeDataFormat, "EDGE_LIST")) {
    ...
    if (!fscanint(ProblemFile, &i)) i = -1;
    while (i != -1) {
      if (i <= 0 || (ProblemType != ATSP ? Dimension : Dimension / 2))
        eprintf("EDGE_DATA_SECTION: Node number out of range: %d", i);
      Ni = &NodeSet[i];
...
static void Read_EDGE_WEIGHT_SECTION() {
  ...
  switch (WeightFormat) {
    case FULL_MATRIX:
      if (ProblemType == ATSP) {
        ...
        for (i = 1; i <= n; i++) {
          Ni = &NodeSet[i];
          for (j = 1; j <= n; j++) {
            if (!fscanint(ProblemFile, &W)) eprintf("EDGE_WEIGHT_SECTION: Missing weight");
    ...
    case UPPER_ROW:
      for (i = 1, Ni = FirstNode; i < Dimension; i++, Ni = Ni->Suc) {
        for (j = i + 1, Nj = Ni->Suc; j <= Dimension; j++, Nj = Nj->Suc) {
          if (!fscanint(ProblemFile, &W)) eprintf("EDGE_WEIGHT_SECTION: Missing weight");
          if (W > INT_MAX / 2 / Precision)
            eprintf("EDGE_WEIGHT_SECTION: "
                    "Weight %d > INT_MAX / 2 / PRECISION",
    ...
    case LOWER_COL:
      for (j = 1, Nj = FirstNode; j < Dimension; j++, Nj = Nj->Suc) {
        for (i = j + 1, Ni = Nj->Suc; i <= Dimension; i++, Ni = Ni->Suc) {
          if (!fscanint(ProblemFile, &W)) eprintf("EDGE_WEIGHT_SECTION: Missing weight");
          if (W > INT_MAX / 2 / Precision)
            eprintf("EDGE_WEIGHT_SECTION: "
                    "Weight %d > INT_MAX / 2 / PRECISION",
