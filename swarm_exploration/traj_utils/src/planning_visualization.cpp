...
PlanningVisualization::PlanningVisualization(ros::NodeHandle& nh) {
  ...
  labeled_frontier_pub_ =
      node.advertise<visualization_msgs::Marker>("/planning_vis/labeled_frontier", 10000);
  pubs_.push_back(labeled_frontier_pub_);

  role_pub_ = node.advertise<visualization_msgs::Marker>("/planning_vis/role", 1);
  pubs_.push_back(role_pub_);

  goal_pub_ = node.advertise<visualization_msgs::Marker>("/planning_vis/current_goal", 1);
  pubs_.push_back(goal_pub_);

  infront_frontiers_pub_ =
      node.advertise<visualization_msgs::Marker>("/planning_vis/infront_frontiers", 1);
  pubs_.push_back(infront_frontiers_pub_);

  trails_tour_pub_ = node.advertise<visualization_msgs::Marker>("/planning_vis/trails_tour", 1);
  pubs_.push_back(trails_tour_pub_);
...
void PlanningVisualization::drawArrow(const Eigen::Vector3d& pos, const double& yaw,
    const Eigen::Vector3d& scale, const Eigen::Vector4d& color, const string& ns, const int& id,
    const int& pub_id) {
  if (pubs_[pub_id].getNumSubscribers() == 0) return;

  visualization_msgs::Marker mk;
  fillBasicInfo(mk, scale, color, ns, id, visualization_msgs::Marker::ARROW);
  mk.action = visualization_msgs::Marker::DELETE;
  pubs_[pub_id].publish(mk);
