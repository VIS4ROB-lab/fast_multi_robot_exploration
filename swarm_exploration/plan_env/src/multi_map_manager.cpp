...
void MultiMapManager::stampTimerCallback(const ros::TimerEvent& e) {
  ...
  for (auto chunks : multi_map_chunks_) {
    plan_env_msgs::IdxList idx_list;
...
void MultiMapManager::chunkCallback(const plan_env_msgs::ChunkDataConstPtr& msg) {
  // Receive chunks from other drones, store them in chunk buffer
  if (msg->from_drone_id == drone_id_) return;
  // if(msg->to_drone_id != drone_id_) return;

  // Check distance
  Eigen::Vector3d other_pos(msg->pos_x, msg->pos_y, msg->pos_z);
  if ((drone_pos_ - other_pos).norm() > communication_range_) {
    return;
  }
...
void MultiMapManager::chunkTimerCallback(const ros::TimerEvent& e) {
  ...
  for (int i = 0; i < chunk_buffer_.size(); ++i) {
    ...
    sort(buffer.begin(), buffer.end(),
        [](const plan_env_msgs::ChunkData& chunk1, const plan_env_msgs::ChunkData& chunk2) {
...
void MultiMapManager::loggingTimerCallback(const ros::TimerEvent& e) {
  ...
  // Store to file
  logger_->log(ros::Time::now().toSec(), total_occupied_volume);
}
